...
if ($userid == '') {
		$result = run_query("SELECT userid, passwd FROM a01sama_users");
	...
	$sql = "UPDATE a01sama_users SET passwd = '$passwd', type = '$type' WHERE userid = '$userid'";
	run_query($sql);
	
	echo '
	<p><a href="index.php?userid='.$userid.'">'.$userid.'</b> was updated for a01sama_users</p>
	<a href="control_panel.php"" class="btn btn-info">Control Panel</a>';
		...
		$result = run_query("SELECT * FROM a01sama_users WHERE userid='$userid'");
